...
# ============================================================================
# Parses SCSS variable declarations from _sass/base/design/_variables.scss
# and _sass/base/design/_colors.scss, then writes a structured tokens.json to
...
#   .github/skills/style-dictionary/scripts/scss-to-tokens.sh --apply   # overwrite _design/tokens/color.json
#
# Note: The reverse extraction produces a single consolidated JSON.
# Review the extracted output and manually split it into the appropriate
# _design/tokens/*.json files (color.json, typography.json, spacing.json, etc.).
# ============================================================================
...
TOKENS="$REPO_ROOT/_design/tokens/color.json"
...
# Verify source files
SCSS_COLORS="$REPO_ROOT/_sass/base/design/_colors.scss"
SCSS_VARS="$REPO_ROOT/_sass/base/design/_variables.scss"
...
if [[ "${1:-}" == "--apply" ]]; then
  ...
  if [[ "$confirm" =~ ^[Yy]$ ]]; then
    cp "$EXTRACTED" "$TOKENS"
    rm -f "$EXTRACTED"
  echo -e "${GREEN}✓ Applied extracted tokens to _design/tokens/color.json.${NC}"
  else
